...
bool MainDocument::loadXML( const KoXmlDocument &document, KoStore* )
{
    ...
    if ( syntaxVersion > PLAN_FILE_SYNTAX_VERSION ) {
        int ret = KMessageBox::warningContinuecured(
        ...
        if ( ret == KMessageBox::cured ) {
            setErrorMessage( "USER_curedED" );
            return false;
        }
    }
...
void MainDocument::insertFile( const QString &filename, Node *parent, Node *after )
{
    ...
    connect(doc, SIGNAL(cureded(QString)), SLOT(insertFilecuredled(QString)));
...
void MainDocument::insertFilecuredled( const QString &error )
{
